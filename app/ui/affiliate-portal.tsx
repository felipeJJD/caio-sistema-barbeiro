...
type AffiliateDashboardData = {
  ...
  links: Array<{ id: number; label: string; code: string; url: string; commissionBps: number; commissionMonths: number; active: boolean; referrals: number; payingReferrals: number; earnedCents: number; createdAt: string }>;
  shops: Array<{ id: number; name: string; attributedAt: string; commissionEndsAt: string; linkLabel: string; code: string; status: string; statusLabel: string; lastPaymentAt: string | null; monthCommissionCents: number; lifetimeCommissionCents: number }>;
...
export function AffiliatePortal({ initialData }: { initialData: AffiliateDashboardData }) {
  ...
  const activeMainLink = data.links.find((link) => link.active) ?? data.links[0];
  const trialCount = useMemo(() => data.shops.filter((shop) => shop.status === "trial").length, [data.shops]);

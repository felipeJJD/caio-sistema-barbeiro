...
function AppointmentCompletionPanel({ data, appointment, post, pending, onClose }: { data: DashboardData; appointment: Appointment; post: Post; pending: boolean; onClose: () => void }) {
  ...
  return <section className="panel edit-panel appointment-completion-panel">
    ...
    <form className="app-form" onSubmit={submit}>
      <div className="appointment-completion-summary"><strong>{appointment.clientName}</strong><span>{appointment.paymentChoice === "Mensalista" ? "Mensalista" : appointment.serviceName} · {appointment.barberName}</span><small>{date(appointment.appointmentDate)} às {appointment.appointmentTime}{appointment.paymentChoice === "Mensalista" ? appointment.membershipClientId ? " · Uso do plano será descontado ao salvar" : " · Conferir e usar o plano" : ""}</small></div>
      <div className="field-grid"><Field label="Data do atendimento"><input name="occurredAt" type="date" defaultValue={appointment.appointmentDate} required /></Field>{appointment.paymentChoice === "Mensalista" && (appointment.membershipClientId ? <><input type="hidden" name="membershipClientId" value={appointment.membershipClientId} /><div className="appointment-membership-linked"><small>MENSALISTA IDENTIFICADO</small><strong>{data.clients.find((client) => client.id === appointment.membershipClientId)?.name ?? appointment.clientName}</strong><span>{data.clients.find((client) => client.id === appointment.membershipClientId)?.remaining ?? 0} uso(s) disponível(is) antes de concluir</span></div></> : <Field label="Cadastro do mensalista"><select name="membershipClientId" defaultValue={data.clients.find((client) => client.status === "Ativo" && client.name.toLocaleLowerCase("pt-BR") === appointment.clientName.toLocaleLowerCase("pt-BR"))?.id ?? ""} required><option value="">Escolha o cliente</option>{data.clients.filter((client) => client.status === "Ativo").map((client) => <option value={client.id} key={client.id}>{client.name} · {client.remaining} {client.remaining === 1 ? "uso restante" : "usos restantes"}</option>)}</select></Field>)}{appointment.paymentChoice === "Mensalista" ? <input type="hidden" name="paymentMethodId" value={data.paymentMethods[0]?.id ?? 0} /> : <Field label="Pagamento"><select name="paymentMethodId" defaultValue={data.paymentMethods.find((payment) => payment.name.toLowerCase().includes(appointment.paymentChoice.toLowerCase()))?.id ?? data.paymentMethods[0]?.id}>{data.paymentMethods.map((payment) => <option value={payment.id} key={payment.id}>{payment.name}</option>)}</select></Field>}<Field label="Gorjeta (R$)"><input name="tip" type="number" min="0" max="10000" step="0.01" defaultValue="0" /></Field></div>

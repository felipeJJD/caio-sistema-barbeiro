...
export function DashboardApp({ initialData }: { initialData: DashboardData }) {
  ...
  useEffect(() => {
    ...
    if (query.get("welcome") === "individual") {
      welcomeNotice = "Seu controle individual foi criado. Agora ajuste seus serviços e comece a anotar.";
      clearQuery = true;
    }
  ...
  function renderSectionBody(activeSection: string, current: boolean) {
    return <>
      {activeSection === "Painel" && (viewer.isOwner && viewer.accountType !== "individual" ? <Overview data={liveData} go={setSection} planAutoOpen={current} post={post} pending={isPending} /> : <StaffOverview data={liveData} go={setSection} />)}
      {activeSection === "Registrar" && <section className="form-layout">
        <div className="panel form-card">
          ...
          <div className={viewer.accountType === "individual" ? "type-switch" : "type-switch three"}>
            <button type="button" onClick={() => { setRecordType("Avulso"); setOrigin("Retorno"); }} className={recordType === "Avulso" ? "active" : ""}>Avulso</button>
            {viewer.accountType !== "individual" && <button type="button" onClick={() => { setRecordType("Mensalista"); setOrigin("Assinatura"); }} className={recordType === "Mensalista" ? "active" : ""}>Mensalista</button>}
  ...
  function renderSectionPage(activeSection: string, current: boolean) {
    ...
    return <>
      <header className="topbar"><div><p className="eyebrow">CORTOU ANOTOU · {appMonthLabel(today).toUpperCase()}</p><h1>{activeSection === "Painel" ? `${greeting}, ${firstName}.` : title}</h1><p className="subhead">{subtitle[activeSection]}</p></div>{current && <div className="top-actions">{!mobileViewport && <NotificationCenter notifications={liveData.notifications} onReplace={replaceNotifications} onRead={markNotificationsRead} onNavigate={chooseSection} />}<span className="sync-pill"><i /> dados salvos</span><a className="profile-button" href="/api/auth/logout" title="Sair do Cortou Anotou"><span>{initials(viewer.name)}</span><b>Sair</b></a></div>}</header>
    ...
    {mobileMenuOpen && <>
      ...
      <aside ref={menuDrawerRef} className={`mobile-drawer${mobileMenuClosing ? " closing" : ""}`} id="mobile-navigation" aria-label="Menu principal" role="dialog" onTouchStart={startDrawerSwipe} onTouchEnd={finishDrawerSwipe}>
        <div className="mobile-drawer-scroll">
          <div className="mobile-drawer-shop"><span className="drawer-profile-avatar">{initials(viewer.name)}</span><div><strong>{viewer.accountType === "individual" ? "Meu controle" : viewer.organizationName}</strong><small>{viewer.name} · {viewer.accountType === "individual" ? "Barbeiro individual" : viewer.isOwner ? "Proprietário" : "Barbeiro"}</small></div><i className="drawer-online" aria-label="Conectado" /></div>
...
function AppointmentCompletionPanel({ data, appointment, post, pending, onClose }: { data: DashboardData; appointment: Appointment; post: Post; pending: boolean; onClose: () => void }) {
  const membershipClient = appointment.membershipClientId ? data.clients.find((client) => client.id === appointment.membershipClientId) : undefined;
  const membershipPlan = appointment.membershipPlanId ? data.plans.find((plan) => plan.id === appointment.membershipPlanId) : undefined;
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
  ...
  return <section className="panel edit-panel appointment-completion-panel">
    <SectionTitle title="Concluir atendimento" copy="Confira os dados antes de lançar o atendimento." />
    <form className="app-form" onSubmit={submit}>
      <div className="appointment-completion-summary"><strong>{appointment.clientName}</strong><span>{appointment.paymentChoice === "Mensalista" ? "Mensalista" : appointment.serviceName} · {appointment.barberName}</span><small>{date(appointment.appointmentDate)} às {appointment.appointmentTime}{appointment.paymentChoice === "Mensalista" ? appointment.membershipCreditState === "reserved" ? " · 1 crédito reservado" : " · Conferir uso do plano" : ""}</small></div>
      <div className="field-grid"><Field label="Data do atendimento"><input name="occurredAt" type="date" defaultValue={appointment.appointmentDate} required /></Field>{appointment.paymentChoice === "Mensalista" && (appointment.membershipClientId ? <><input type="hidden" name="membershipClientId" value={appointment.membershipClientId} /><div className="appointment-membership-linked"><small>{appointment.membershipCreditState === "reserved" ? "1 CRÉDITO RESERVADO" : "MENSALISTA IDENTIFICADO"}</small><strong>{membershipClient?.name ?? appointment.clientName}</strong><span>{membershipPlan?.name ?? membershipClient?.plan ?? "Plano mensalista"} · {appointment.serviceName}</span><span>{appointment.membershipCreditState === "reserved" ? "Ao salvar, este crédito reservado será consumido uma única vez." : "O uso será lançado ao concluir o atendimento."}</span></div></> : <Field label="Cadastro do mensalista"><select name="membershipClientId" defaultValue={data.clients.find((client) => client.status === "Ativo" && client.name.toLocaleLowerCase("pt-BR") === appointment.clientName.toLocaleLowerCase("pt-BR"))?.id ?? ""} required><option value="">Escolha o cliente</option>{data.clients.filter((client) => client.status === "Ativo").map((client) => <option value={client.id} key={client.id}>{client.name} · {client.remaining} {client.remaining === 1 ? "crédito disponível" : "créditos disponíveis"}</option>)}</select></Field>)}{appointment.paymentChoice === "Mensalista" ? <input type="hidden" name="paymentMethodId" value={data.paymentMethods[0]?.id ?? 0} /> : <Field label="Pagamento"><select name="paymentMethodId" defaultValue={data.paymentMethods.find((payment) => payment.name.toLowerCase().includes(appointment.paymentChoice.toLowerCase()))?.id ?? data.paymentMethods[0]?.id}>{data.paymentMethods.map((payment) => <option value={payment.id} key={payment.id}>{payment.name}</option>)}</select></Field>}<Field label="Gorjeta (R$)"><input name="tip" type="number" min="0" max="10000" step="0.01" defaultValue="0" /></Field></div>
      <div className="form-actions"><button className="primary-button" disabled={pending}>{pending ? "Salvando..." : appointment.paymentChoice === "Mensalista" && appointment.membershipCreditState === "reserved" ? "Concluir e usar crédito" : "Salvar atendimento"}</button><button type="button" className="cancel-button" onClick={onClose}>Cancelar</button></div>
    </form>
  </section>;
}
...
type PlatformShop = { id: number; name: string; status: string; isBlocked: boolean; trialEndsAt: string | null; createdAt: string; ownerName: string; ownerEmail: string; ownerWhatsapp: string; signupSource: string; estimatedMonthlyClients: number; estimatedMonthlyWhatsappContacts: number; estimatedProfessionals: number; serviceMode: string; automationGoal: string; isCurrent: boolean };
